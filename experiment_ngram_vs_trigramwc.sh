...
sizes="10000 100000 500000 1000000 2000000 4000000 8000000 15000000";
...
for size in $sizes; do
	...
	echo "shuf -n $size corpus/factored_corpus_WGNCPL.txt > $tmp_factored_corpus";

	echo "$(date): train flm" | tee -a $logfile;
	echo $fngram_count_command -text $tmp_factored_corpus;

	echo "$(date): extract words only" | tee -a $logfile;

	echo "sed -E 's/([^ :]*):([^ ])*/\1/g' < $tmp_factored_corpus > $tmp_corpus";

	echo rm -f $tmp_factored_corpus;

	echo "$(date): train ngram" | tee -a $logfile;

	echo $ngram_count_command -text $tmp_corpus;

	echo "perplexities:"

	echo "python predictor_tables.py -ngram_lm $four_gram_lm_file -ngram_predictor_orders 4 -flm_model_filenames flm_models/trigramWC.flm | python analyze_perplexities --terminal  | tee -a $logfile"

done

echo rm -f flm_models/trigramWC.count.gz flm_models/trigramWC.lm.gz;
echo rm -f $four_gram_lm_file;
